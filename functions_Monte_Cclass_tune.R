...
evaluateModels <- function(fnameX,fnameY,m) {
  ...
  m1 <- lm( y_GT_C1 ~ alpha_exp_fft_blos+alpha_exp_fft_br+decay_index_blos+decay_index_br+
  ...
  plot(te$y_GT_C1,xlab="observation",ylab="value of y",main=paste("lm model training original split",sep=""))
  ...
  m2 <- glm( y_GT_C1 ~ alpha_exp_fft_blos+alpha_exp_fft_br+decay_index_blos+decay_index_br+
               ising_energy_blos+ising_energy_br+ising_energy_part_blos+ising_energy_part_br+
  ...
  plot(te$y_GT_C1,xlab="observation",ylab="value of y",main=paste("glm training with probit link original split ",sep=""))
  ...
  m3 <- glm( y_GT_C1 ~ alpha_exp_fft_blos+alpha_exp_fft_br+decay_index_blos+decay_index_br+
               ising_energy_blos+ising_energy_br+ising_energy_part_blos+ising_energy_part_br+
  ...
  plot(te$y_GT_C1,xlab="observation",ylab="value of y",main=paste("glm training with logit link original split ",sep=""))
  ...
  teRF <- rbind(te,teTst)
  trainRF <- 1:dim(te)[1]
  p.randomForest <- randomForest(as.factor(y_GT_C1) ~ alpha_exp_fft_blos+alpha_exp_fft_br+decay_index_blos+decay_index_br+
                                   ising_energy_blos+ising_energy_br+ising_energy_part_blos+ising_energy_part_br+
  ...
  p.svm <- svm(factor(y_GT_C1,levels=factor(c("0","1"))) ~ alpha_exp_fft_blos+alpha_exp_fft_br+decay_index_blos+decay_index_br+
                 ising_energy_blos+ising_energy_br+ising_energy_part_blos+ising_energy_part_br+
                 mpil_blos+mpil_br+nn_currents+r_value_blos_logr+r_value_br_logr+wlsg_blos+wlsg_br,data=te,
  ...
  plot(as.numeric(te$y_GT_C1),xlab="observation",ylab="value of y",main=paste("SVM with probability option, combined regressors\n", "Training, Original Split",sep=" "))
  #lines(p.svm$fitted, col=6)
  ...
  p.nnet <- nnet(cbind(te$alpha_exp_fft_blos,te$alpha_exp_fft_br,te$decay_index_blos,te$decay_index_br,
                       ...
                       te$mpil_blos,te$mpil_br,te$nn_currents,
                       te$r_value_blos_logr,te$r_value_br_logr,te$wlsg_blos,te$wlsg_br),
                 te$y_GT_C1, size = 4, decay = 10^(-1),
                 entropy=T,maxit=2000,MaxNWts=2000  )
  
  p <- p.nnet
  jpeg(paste("MLP_model_combined_DATABASE_trn_orig_split",".jpeg",sep=""))
  plot(te$y_GT_C1,xlab="observation",ylab="value of y",main=paste("MLP with entropy option, combined regressors\n", "Training, Original Split",sep=" "))
  ...
  plot(teTst$y_GT_C1,xlab="observation",ylab="value of y",main=paste("MLP with entropy option, combined regressors,\n",
  ...
  for (thresHOLD in seq(0.00,1.00,0.01) )
  {
    ...
    cfmat <- table(p.nnet.predict.01, teTst$y_GT_C1)
    ...
    cfmat <- table(m1.predict.01, teTst$y_GT_C1)
    ...
    cfmat <- table(m2.predict.01, teTst$y_GT_C1)
    ...
    cfmat <- table(m3.predict.01, teTst$y_GT_C1)
    ...
    cfmat <- table(p.randomForest.predict.01, teTst$y_GT_C1)
    ...
    cfmat <- table(p.svm.predict.01, teTst$y_GT_C1)
  ...
  pred0 <- prediction(p.nnet.predict,teTst$y_GT_C1)
  ...
  pred1 <- prediction(m1.predict,teTst$y_GT_C1)
  ...
  pred2 <- prediction(m2.predict,teTst$y_GT_C1)
  ...
  pred3 <- prediction(m3.predict,teTst$y_GT_C1)
  ...
  pred4 <- prediction(p.randomForest.predict.new,teTst$y_GT_C1)
  ...
  pred5 <- prediction(p.svm.predict.num,teTst$y_GT_C1)
  ...
  mod0 <- verify(obs = teTst$y_GT_C1, pred = p.nnet.predict,thresholds=seq(0,1,0.05))
  ...
  sd0 <- (p.nnet.predict - teTst$y_GT_C1)^2
  brier0 <- mean(sd0)
  ...
  mod1 <- verify(obs = teTst$y_GT_C1, pred = m1.predict.new,thresholds=seq(0,1,0.05))
  ...
  #teTst$yGTM1
  #https://en.wikipedia.org/wiki/Brier_score
  sd1 <- (m1.predict - teTst$y_GT_C1)^2
  brier1 <- mean(sd1)
  ...
  mod2 <- verify(obs = teTst$y_GT_C1, pred = m2.predict,thresholds=seq(0,1,0.05))
  ...
  #teTst$yGTM1
  #https://en.wikipedia.org/wiki/Brier_score
  sd2 <- (m2.predict - teTst$y_GT_C1)^2
  brier2 <- mean(sd2)
  ...
  mod3 <- verify(obs = teTst$y_GT_C1, pred = m3.predict,thresholds=seq(0,1,0.05))
  ...
  #teTst$yGTM1
  #https://en.wikipedia.org/wiki/Brier_score
  sd3 <- (m3.predict - teTst$y_GT_C1)^2
  brier3 <- mean(sd3)
  
  
  #randomForest model
  #mod4 <- verify(obs = teTst$yIN_C1_C9, pred = p.randomForest.predict.new)
  mod4 <- verify(obs = teTst$y_GT_C1, pred = p.randomForest.predict.new,thresholds=seq(0,1,0.05))
  ...
  #teTst$yGTM1
  #https://en.wikipedia.org/wiki/Brier_score
  sd4 <- (p.randomForest.predict - teTst$y_GT_C1)^2
  brier4 <- mean(sd4)
  ...
  mod5 <- verify(obs = teTst$y_GT_C1, pred = p.svm.predict.num,thresholds=seq(0,1,0.05))
  ...
  #teTst$yGTM1
  #https://en.wikipedia.org/wiki/Brier_score
  sd5 <- (p.svm.predict.num - teTst$y_GT_C1)^2
  brier5 <- mean(sd5)
  ...
  BSref <- mean((sum(teTst$y_GT_C1) / length(teTst$y_GT_C1) - teTst$y_GT_C1)^2)   
