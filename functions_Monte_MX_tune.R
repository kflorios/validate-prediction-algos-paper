...
evaluateModels <- function(fname,m) {
  ...
  for (i in 1:dim(dataMatf)[1]) {
    ...
    #y_C1_C9_flare[i] <- as.numeric(dataMatf[i,11] )
  }
  ...
  m1 <- lm( yGTM1 ~ logRBl+logRBr+FSPIBl+FSPIBr+TLMPILBl+TLMPILBr+
  ...
  m2 <- glm( yGTM1 ~ logRBl+logRBr+FSPIBl+FSPIBr+TLMPILBl+TLMPILBr+
  ...
  m3 <- glm( yGTM1 ~ logRBl+logRBr+FSPIBl+FSPIBr+TLMPILBl+TLMPILBr+
  ...
  teRF <- rbind(te,teTst)
  trainRF <- 1:dim(te)[1]
  p.randomForest <- randomForest(yGTM1 ~ logRBl+logRBr+FSPIBl+FSPIBr+TLMPILBl+TLMPILBr+
  ...
  #                  te$yGTM1,gamma = 2^(-3:0), cost = 2^(0:3), probability=TRUE)
  ...
  #                  te$yGTM1,gamma = 0.125, cost = 8, probability=TRUE)
  ...
  #for(tries in 1:1) {
    ...
    #for (iNode in 16:16) {
      ...
      #                          te$DIBl,te$DIBr,te$BeffBl,te$BeffBr,te$IsinEn1Bl,te$IsinEn1Br,te$IsinEn2Bl,te$IsinEn2Br,te$NNC),
      #                    te$yGTM1, size = 8*2^(-1:2), decay = 10^(-3:-1),
      ...
      for (thresHOLD in seq(0.00,1.00,0.01) )
      {
        ...
        plot(teTst$yGTM1,xlab="observation",ylab="value of y",main=paste("random Forest model, combined regressors","\n",
                                                                         "Testing, Original Split",sep=" "))
        ...
        cfmat <- table(p.randomForest.predict.01, teTst$yGTM1)
      ...
      pred4 <- prediction(p.randomForest.predict.new,teTst$yGTM1)
      ...
      #randomForest model
      #mod4 <- verify(obs = teTst$yGTM1, pred = p.randomForest.predict.new)
      mod4 <- verify(obs = teTst$yGTM1, pred = p.randomForest.predict.new,thresholds=seq(0,1,0.05))
...
makePlots1 <- function(teTst) {
 ...
 for (i in 1:n) {
   x=te$DI[i]
   y=te$Beff[i]
   #if (as.numeric(teTst$yGTM1[i])==0) { points(x,y,col="blue") }
   if (as.numeric(teTst$yGTM1[i])==1) { points(x,y,col="red") }
 }
